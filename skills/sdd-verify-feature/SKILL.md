...
3. **Dispatch Parallel Subagents**:
   - In a single response block, dispatch the following subagents to run concurrently:
     ...
     - **Security Subagent (persona: `security-auditor`)** *(Only if user approved the security audit in the previous step)*: Instruct it to apply the `agent-skills:security-auditor` skill to the feature diff against the target base branch to identify vulnerabilities.
