...
2. **Read & Parse Feature Spec**:
   ...
   - Parse the feature spec sections to extract the planning details:
     ...
     - **UI Design Reference**: if a `figma.com` URL is present in `UI Design Reference`, carry it verbatim into `requirements.md` under a `## UI Design Reference` section. Do not paraphrase it.
...
- **targetBaseBranch metadata**: Determine the current git branch in the repository at the time of planning. Inject `targetBaseBranch: <current-branch>` into the YAML frontmatter at the top of `plan.md` so that future verification subagents know exactly where to merge this feature branch back into.
